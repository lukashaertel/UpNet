...
namespace UpNet.Graphics
{
    ...
    {
        public Span<byte> Permutations { get; }

        public float Center { get; }

        public float Range { get; }

        public SimplexNoise(Span<byte> permutations, float center = 0f, float range = 1f)
        {
        ...
        public float Eval(float x) =>
            Center + Generate(x) * Range;

        public float Eval(float x, float y) =>
            Center + Generate(x, y) * Range;

        public float Eval(float x, float y, float z) =>
            Center + Generate(x, y, z) * Range;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private byte GetPermutation(int value) =>
            Permutations[Mod256(value)];

        private float Generate(float x)
        {
            const float scale = 0.395f;
            ...
            var t0 = 1.0f - x0 * x0;
            t0 *= t0;
            var n0 = t0 * t0 * Grad(GetPermutation(i0 & 0xff), x0);

            var t1 = 1.0f - x1 * x1;
            t1 *= t1;
            var n1 = t1 * t1 * Grad(GetPermutation(i1 & 0xff), x1);
        ...
        private float Generate(float x, float y)
        {
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * Grad(GetPermutation(ii + GetPermutation(jj)), x0, y0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * Grad(GetPermutation(ii + i1 + GetPermutation(jj + j1)), x1, y1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * Grad(GetPermutation(ii + 1 + GetPermutation(jj + 1)), x2, y2);
            }
        ...
        private float Generate(float x, float y, float z)
        {
            ...
            {
                t0 *= t0;
                n0 = t0 * t0 * Grad(GetPermutation(ii + GetPermutation(jj + GetPermutation(kk))), x0, y0, z0);
            }
            ...
            {
                t1 *= t1;
                n1 = t1 * t1 * Grad(GetPermutation(ii + i1 + GetPermutation(jj + j1 + GetPermutation(kk + k1))), x1, y1,
                    z1);
            }
            ...
            {
                t2 *= t2;
                n2 = t2 * t2 * Grad(GetPermutation(ii + i2 + GetPermutation(jj + j2 + GetPermutation(kk + k2))), x2, y2,
                    z2);
            }
            ...
            {
                t3 *= t3;
                n3 = t3 * t3 * Grad(GetPermutation(ii + 1 + GetPermutation(jj + 1 + GetPermutation(kk + 1))), x3, y3,
                    z3);
            }
